...
//푸터
	...
	box.forEach(function(item){
		...
		wrap.style.height =`${(box.length * boxHeight)}px`;
//각 box의 Top위치 배열에 담고, wrap의 높이 설정
...
//box4에 도달하면, 모든 a태그 색상 속성값 변경
			...
			if(rollY > boxInfo[4]){
				item.style.setProperty('--bgColor2', '#111');
				footer.style.bottom = '0';
			}else{
				item.style.setProperty('--bgColor2', '#ead5d3');
...
//스크롤시, 움직이는 parallax 효과
		...
		// });
		// pg[2].addEventListener('click',function(e){
		// 	e.preventDefault();
